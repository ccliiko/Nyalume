...
| 用户导入的 PMX 模型与第三方 VMD 动作 | 来源及条款因作者而异 | **不随包分发**：已知部分素材禁止二配或商用。**拍演示视频是另一回事**——多数条款允许视频用途、要求署名，个人发布可行，商业宣发才需另行授权 |
...
3D 素材的具体限制、演示视频和宣发边界见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 的素材一节。本机能加载某个素材不构成对其发行或广告使用的授权。

> 本文件是内部合规记录，**只随源码仓库提供，不进入发行包**（发行包里对应的内容在
> `THIRD_PARTY_NOTICES.md`）。公开包里不放它的另一个原因：里面的"已知生成链/未批准"结论是
> 给我们自己复核用的，不该出现在用户拿到的目录里。
