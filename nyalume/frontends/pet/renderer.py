...
def _resize_for_colorkey(im: PILImage.Image, size: tuple[int, int]) -> PILImage.Image:
    """Resize without leaving semi-transparent pixels that Windows shows as a halo."""
    if im.size == size:
        return im
    # Color-key windows cannot display antialiased alpha correctly. Keep the
    # source silhouette instead of thresholding the Lanczos mask, which grows
    # a bright one-pixel fringe around pale characters.
    mask = im.getchannel("A").resize(size, PILImage.NEAREST)
    scaled = _unpremul(_premul(im).resize(size, PILImage.LANCZOS))
    scaled.putalpha(mask)
    return _harden(scaled)
...
class PetWindow:
    ...
    def __init__(
        ...
        self.pet = pet
        self.scale = max(0.5, min(1.0, float(pet.get("scale", 1.0))))
        self.w = max(80, round(int(pet.get("width", 150)) * self.scale))
        self.h = max(80, round(int(pet.get("height", 150)) * self.scale))
    ...
    def _enable_dnd(self) -> None:
        """接收 Windows 资源管理器拖进来的文件夹（桌宠“吃文件夹”入口）。"""
        try:
            from tkinterdnd2 import DND_FILES

            self.canvas.drop_target_register(DND_FILES)
            self.canvas.dnd_bind("<<Drop>>", self._dnd_on_drop)
        except Exception:
            pass  # 库缺失/环境不支持时只是没有拖放，不影响其他功能

    def _dnd_on_drop(self, event) -> None:
        try:
            paths = [str(p) for p in self.win.tk.splitlist(event.data or "")]
        except Exception:
            paths = [str(event.data or "")]
    ...
    def eat(self, ms: int = 3000) -> None:
        """吃文件夹动画：短暂进入 eating 帧组（多帧由 _animate 循环驱动）。"""
        self.poke()
        self._eat_until = time.time() + max(500, int(ms))
    ...
    def tap_head(self, meowed: bool = False) -> None:
        """聊天窗敲头联动：轻弹一下；触发喵时同步冒泡。"""
        self.poke()
        for job in self._tap_jobs:
            try:
                ...
                pass
        self._tap_jobs.clear()
        self._set_tap_offset(3)
        for delay, offset in ((70, -4), (145, 2), (240, 0)):
            self._tap_jobs.append(
                self.win.after(delay, lambda value=offset: self._set_tap_offset(value))
            )
        if meowed:
            self.cheer("喵~")

    def _set_tap_offset(self, offset: int) -> None:
    ...
    def _show_speech(self, text: str, ms: int) -> None:
        ...
        try:
            ...
            pass
        c = tk.Canvas(
            speech, width=1, height=1, bg=_TRANSPARENT,
            highlightthickness=0, bd=0,
        )
        c.pack()
        # 单层原生文字比九层描边文字清楚；浅色气泡保证复杂桌面上也可读。
        self._speech_font = tkfont.Font(
            root=speech, family="Microsoft YaHei UI", size=10, weight="bold",
        )
        text_id = c.create_text(
            0, 0, text=text, fill="#5b3a66", font=self._speech_font,
            width=min(360, max(120, speech.winfo_screenwidth() - 40)), anchor="nw",
        )
        box = c.bbox(text_id) or (0, 0, 70, 18)
        sw = max(90, box[2] - box[0] + 28)
        sh = max(36, box[3] - box[1] + 22)
        c.config(width=sw, height=sh)
        body = c.create_rectangle(
            1, 1, sw - 2, sh - 8,
            fill="#fffdfd", outline="#f2a6bd", width=1,
        )
        tail = c.create_polygon(
            sw / 2 - 6, sh - 9, sw / 2 + 6, sh - 9, sw / 2, sh - 2,
            fill="#fffdfd", outline="#f2a6bd",
        )
        c.coords(text_id, sw / 2, (sh - 8) / 2 + 1)
        c.itemconfigure(text_id, anchor="center")
        c.tag_lower(body, text_id)
    ...
    def _peek_size(self, axis: str) -> tuple[int, int]:
        ...
        try:
            ...
            scale = (_DOCK_FOOTPRINT * self.scale) / max(1, max(w, h))
    ...
    def _head_size(self) -> tuple[int, int]:
        ...
        if paths and PILImage is not None:
            with PILImage.open(paths[0]) as im:
                w, h = im.size
            l, t, r, b = _HEAD_BOX
            return (max(60, int((r - l) * w * self.scale)),
                    max(60, int((b - t) * h * self.scale)))
        return int(self.w * 0.62), int(self.h * 0.52)
    ...
    def _group_name(self) -> str:
        """当前要播放的分组：吃文件夹 > 工作态 > 心情档；缺分组回退 idle。"""
        frames = self.pet.get("frames") or {}
        if time.time() < self._eat_until and "eating" in frames:
            return "eating"
    ...
    def _animate(self) -> None:
        ...
        self._draw()
        now = time.time()
        if now < self._reminder_until:
            interval = 130
        elif now < self._eat_until or self.working:
            interval = 150  # 咀嚼/打字需要比待机更快的帧率
        else:
            interval = _ANIM_MS
    ...
    def _draw(self) -> None:
        ...
        if paths:
            ...
            if path not in self._bounds and PILImage is not None:
                try:
                    with PILImage.open(path) as im:
                        box = im.convert("RGBA").getbbox()
                        if box:
                            sx, sy = self.w / im.width, self.h / im.height
                            box = tuple(round(v * (sx if i % 2 == 0 else sy))
                                        for i, v in enumerate(box))
                        self._bounds[path] = box
                except Exception:
                    self._bounds[path] = None
            if (
                self._dragging
                ...
                if im is None:
                    with PILImage.open(path) as im0:
                        im = im0.convert("RGBA")
                    im = _resize_for_colorkey(im, (self.w, self.h))
            ...
            if self.docked:
                cx, cy = photo.width() / 2, photo.height() / 2 + self._tap_offset
            else:
                cx, cy = self.w / 2, self.h / 2 + self._tap_offset
            self.canvas.create_image(
                cx, cy, image=photo, tags="pet"
            )
        else:
            self._draw_procedural(group)
        if self.working and not self.docked:
            # 单张 working 素材也有清晰动效：三个点随帧轮流亮起。
            active = self._tick % 3
            for i in range(3):
                self.canvas.create_oval(
                    ...
                    self.w / 2 - 10 + i * 12,
                    self.h - 28,
                    fill="#f472b6" if i == active else "#fbcfe8",
                    outline="",
                    tags="work-fx",
                )
    ...
    def _apply_idle_bob(self) -> None:
        ...
        if (
            self._grab_active
            or self._dragging
            or time.time() - self._press_at < 0.35
        ):
            return  # 点击/拖拽瞬间不挪窗口，避免“瞬移”感
        now = time.time()
        if now < self._eat_until and not self.docked and not self._dragging:
            # 吃文件夹：小幅上下抖 + 左右摆，配合嘴部帧模拟“啊呜啊呜”
            ax, ay = self.win.winfo_x(), self.win.winfo_y()
            wob = int(math.sin(self._tick * 1.1) * 3)
            self.win.geometry(f"+{ax + wob}+{ay + (2 if self._tick % 2 else -2)}")
    ...
    def _draw_procedural(self, group: str) -> None:
        ...
        cx = w / 2
        r = min(w, h) * 0.30
        cy = h / 2 + r * 0.15 + (2 if self._tick % 2 else -2) + self._tap_offset  # 轻微上下晃
