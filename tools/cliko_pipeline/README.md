...
python tools/cliko_pipeline/make_moods.py

# 2) 逐个排队出图
python tools/cliko_pipeline/comfy_submit.py user_pets/cliko/art/moods/happy_workflow_api.json
python tools/cliko_pipeline/comfy_submit.py user_pets/cliko/art/moods/love_workflow_api.json

# 3) 等出图
python tools/cliko_pipeline/comfy_wait.py <prompt_id...>

# 4) 把 ComfyUI output 里的 cliko_mood_<key>_*.png 拷到
#    user_pets/cliko/moods_raw/<key>.png，然后抠成 300x300 透明帧：
python tools/cliko_pipeline/pet_skin_cut.py \
  user_pets/cliko/moods_raw/happy.png --prefix mood_happy_ --out user_pets/cliko

# 5) 把 mood_<key>_1.png 改名成 user_pets/cliko/mood_<key>.png，
#    并在 user_pets/cliko/manifest.json 的 frames 里填上对应文件
```
