...
- 3D 桌宠：加载用户自备的 PMX 模型与 VMD 动作，支持按部位互动、拖拽、
  ...
  用 `pet_perform` 指挥舞蹈、表情、视线和气泡，`pet_configure` 调整风格、模型、
  安静与搭话档位；聊天状态侧栏可读取桌宠概况
- 桌宠动作兼容性检查可从聊天 Agent 或状态面板发起；共同经历面板支持记录、确认和完成小约定，保存在本地
- 聊天输入支持 Enter 发送、Shift+Enter 换行；回复中的 Markdown 表格直接呈现为表格

本轮桌宠接口说明、陪伴方向与 backlog 评估见 [桌宠互动与世界观](docs/desktop-companion-plan.md)。
...
每份 PMX 可独立设置缩放、朝向、表情映射与动作白名单，滚轮大小和旋转角度自动保存。
右键配置页可打开当前模型配置；骨骼兼容检查与字段说明见 [每模型配置](docs/model-profiles.md)。
...
游戏角色模型、非商用或禁止二配的 VMD 动作不得因为出现在本机 3D 桌宠中就进入公开 EXE
或商业宣发（卖软件、带货、广告、商店页）；署名和免责声明不能代替授权。
**拍演示视频是另一回事**：多数素材条款允许视频用途、只要求署名并禁止商业使用，
免费发布的桌宠演示视频按各自条款署名即可，具体见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 的素材一节。
